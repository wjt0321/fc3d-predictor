...
1. **独立专家候选**：每位专家按位打分，各自产出 top-3 笛卡尔积（~27个/专家）
2. **聚合候选**：`aggregate_scores()` 融合 → top-6（216个）
3. **马尔可夫联合概率候选**：`generate_markov_candidates()` 链式法则 P(d1)×P(d2|d1)×P(d3|d1,d2) + Laplace 平滑 → top-300 完整 3 位数（候选池主导）
4. **历史热号**：最近 200 期高频号码 top-80
5. **历史相似期匹配**：找最近 5 期最相似的历史片段，取后续开奖号 top-3
6. **合并去重 → MMR 选择**：λ 自适应（0.45→0.05），强制保留 top-2 马尔可夫候选，确保≥9数字覆盖
7. **评分维度**：位置分 + 和值 + 跨度趋势 + 形态感知 + 平衡 + 重复惩罚
